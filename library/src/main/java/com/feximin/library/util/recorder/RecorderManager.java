...
//                    mListener.onMaxDuration(mConfig.getMaxDuration());
	...
	public void stop() {
		...
		if (duration < mConfig.getMinDuration()) {
			mCurrentAudioFile.delete();
			mCurrentAudioFile = null;
			mListener.onShortDuration(duration);
		} else {
            long length = mCurrentAudioFile.length();
            if (!mCurrentAudioFile.exists() || length == 0) {
                mListener.onError();
            } else {
                mListener.onRecordFinish(duration, mCurrentAudioFile.getAbsolutePath());
            }
		}
	}
